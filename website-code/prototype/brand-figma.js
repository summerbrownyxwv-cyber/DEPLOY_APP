...
(() => {
  ...
  const timeline=history=>`<section class="xf-section xf-history" id="history"><header class="xf-heading"><div><h2>企业历程</h2><p>让积累，成为下一步的基础。</p></div></header><div class="xf-years" aria-label="企业发展年份">${history.map(([year])=>{const index=history.findIndex(item=>item[0]===year);return `<button type="button" data-year="${index}" ${index<0?'disabled title="设计稿未提供该年事件"':''} aria-pressed="${index===0}">${year}</button>`;}).join('')}</div><div class="xf-history-viewport"><div class="xf-history-track">${history.map(([year,,copy],i)=>`<button class="xf-history-card ${i===0?'is-active':''}" type="button" data-history="${i}" aria-pressed="${i===0}"><span class="xf-history-visual">${i===0?image('imgKhp016632','2015年艾灸器1.0'):`<img src="../../assets/review-20260920/history-${year}.webp" alt="${year}年企业历程" width="1200" height="720" loading="lazy">`}</span><span class="xf-history-copy"><time>${year}</time><span>${copy}</span></span></button>`).join('')}</div></div></section>`;
  const group=()=>`<section class="xf-section xf-group"><h2>东元集团</h2><p class="xf-group-sub">寻艾X艾艾贴</p><div class="xf-group-grid"><figure>${image('imgRectangle','寻艾艾灸馆品牌标识')}<figcaption>「东元集团」旗下终端品牌 革新传统艾灸服务</figcaption></figure><figure><figcaption>「东元集团」旗下自有供应链品牌<br>艾灸行业十年深度耕耘</figcaption><img src="../../assets/review-20260920/group.webp" alt="艾艾贴育苗研究基地品牌标识" width="1000" height="1100" loading="lazy"></figure></div></section>`;
  ...
  window.XunAiFigmaBrand={
    render(evidence){
      const events={2015:'售出第一盒艾灸器1.0；自此成为品牌纪念',2016:'在艾都蕲春成立生产基地',2017:'艾灸器2.0正式发布',2019:'研发、生产、仓储、一体化新园区正式落成',2020:'与中国中医科学院中药研究所签约启动，艾草种苗及基因稳定性研究',2021:'艾草行业领先的智能化提取产线正式投产',2022:'与蕲春政府及行业协会，共同成立蕲艾产业技术研究院为行业赋能',2023:'艾草新品种[蕲优一号]培育并注册成功',2024:'获得全国首张《品质中药材 艾叶》认证证书',2025:'寻艾升级,盈利跑通，快速复制启动千城万店'};
    ...
    mount(main){
      ...
      let selectedYear=0;
      const historyPhone=matchMedia('(max-width:767.98px)'),historyTablet=matchMedia('(min-width:768px) and (max-width:1023.98px)');
      const positionYear=()=>{const offset=cards[selectedYear].offsetLeft-cards[0].offsetLeft;historyTrack.style.transform=historyTablet.matches?'none':`translateX(-${offset}px)`;if(historyPhone.matches)historyTrack.style.setProperty('--mobile-history-offset',`-${offset}px`);else historyTrack.style.removeProperty('--mobile-history-offset');};
      ...
      const revealYear=()=>{if(historyPhone.matches){const list=years[0].parentElement,button=years[selectedYear];list.scrollTo({left:button.offsetLeft-list.offsetLeft-(list.clientWidth-button.offsetWidth)/2,behavior:reduced()?'instant':'smooth'});}};
      years.filter(button=>!button.disabled).forEach(button=>button.addEventListener('click',()=>{setYear(Number(button.dataset.year));revealYear();if(historyTablet.matches)cards[selectedYear].scrollIntoView({block:'center',behavior:reduced()?'instant':'smooth'});}));cards.forEach((card,i)=>card.addEventListener('click',()=>{setYear(i);revealYear();}));
      historyTrack.addEventListener('keydown',event=>{const mobile=historyTablet.matches;if(!(mobile?['ArrowUp','ArrowDown']:['ArrowLeft','ArrowRight']).includes(event.key))return;event.preventDefault();const i=cards.findIndex(card=>card.classList.contains('is-active'));const next=Math.max(0,Math.min(cards.length-1,i+(['ArrowRight','ArrowDown'].includes(event.key)?1:-1)));setYear(next);revealYear();cards[next].focus({preventScroll:true});if(mobile)cards[next].scrollIntoView({block:'nearest',behavior:'instant'});});
      let historyTouch=null;historyTrack.addEventListener('touchstart',event=>{historyTouch=[event.touches[0].clientX,event.touches[0].clientY];},{passive:true});historyTrack.addEventListener('touchend',event=>{if(historyTablet.matches||!historyTouch)return;const dx=event.changedTouches[0].clientX-historyTouch[0],dy=event.changedTouches[0].clientY-historyTouch[1];if(Math.abs(dx)>60&&Math.abs(dx)>Math.abs(dy)){setYear(Math.max(0,Math.min(cards.length-1,selectedYear+(dx<0?1:-1))));revealYear();}historyTouch=null;},{passive:true});
